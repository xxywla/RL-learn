...
def evaluate(model, tokenizer, device, dtype):
    ...
    test_dataset = CountdownTasksDataset(
        data_path=str(config.COUNTDOWN_TASKS_PATH),
        tokenizer=tokenizer,
        split="test",
...
def main():
    pretrained_model_path = config.QWEN2_5_3B_INSTRUCT_PATH
    ...
    tokenizer = Tokenizer(str(config.QWEN2_5_3B_INSTRUCT_PATH) + "/tokenizer.json")
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        if step % 10 == 0:
            ...
            print(f"\r评估数据集回答正确率:{eval_success_rate:.2f}" + " " * 100)
